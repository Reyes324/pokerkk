...
**任何涉及 UI / 视觉 / 排版 / 组件设计的工作，第一步必须调用 `impeccable` skill，并严格按照它的命令流程执行（shape → craft），不得跳过。**

- 涉及设计就调用，不论改动大小——徽标样式、颜色方案、新组件、布局调整均算在内
- 调用后必须真正执行 skill 规定的流程（读 reference、写 shape brief、做 craft），不能仅 invoke 后绕过
- 先做 shape（UX/UI 规划），再做 craft（落地代码），不允许跳过 shape 直接写代码
- 设计决策必须基于 DESIGN_SYSTEM.md 的已有 token 和组件；新增 token 前须告知用户
